...
const Home: React.FC<Props> = (props: Props) => {
  ...
  // useStateで状態の保存場所を定義する 
  // 参考：(https://ja.reactjs.org/docs/hooks-state.html)
  //const [controllersResult, setControllers] = React.useState<ControllersResult | undefined>(undefined);
  const [controllersResult, setControllers] = useState<ControllersResult | undefined>(undefined);

  // useEffectでコンポーネントのレンダリング後にfetchControllersを叩いてAPIから取得する
  // 参考：(https://ja.reactjs.org/docs/hooks-effect.html)
  React.useEffect(() => {

    //処理に成功するとcontrollersResultに値が入る
    fetchControllers(setControllers);
    
  }, []);
   // 第2引数に空の配列を渡してあげることで、
   //stateが更新されてもレンダリングが走らないようにする。 (無限ループ対策)
