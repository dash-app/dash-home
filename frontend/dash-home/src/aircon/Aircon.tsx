...
const AirconPanel: React.FC<ControllerProps> = props => {
  ...
  const [stash, setStash] = useState<AirconState>(Object.assign({ ...props.controller.appliances?.aircon! }));
  const [aircon, setAircon] = useState<AirconState>(props.controller.appliances?.aircon!);
...
const update = (state: AirconState, after?: any) => {
  ...
  props.sendTimer(() => {
    props.setSending();
    sendAircon(props.controller.id, stateToEntry(state), setResult);
  }, 300);
}
